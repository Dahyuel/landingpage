...
<div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
  ...
  {/* Background Image Section - starts after logos, ends before 5 Days section */}
  <div
          ...
          <div className="relative max-w-6xl mx-auto">
            <div className="flex items-center justify-center gap-2 sm:gap-4 md:gap-6">
              ...
              <div className="flex-1 overflow-hidden px-2">
                <div className="flex justify-center items-center gap-2 sm:gap-4 md:gap-6 min-h-[150px] sm:min-h-[200px] flex-wrap md:flex-nowrap">
                  {Array.from({ length: window.innerWidth < 640 ? 2 : window.innerWidth < 768 ? 3 : 5 }).map((_, index) => {
                    ...
                    return (
                      ...
                      >
                        <div className="relative group">
                          <div className="absolute inset-0 bg-gradient-to-r from-orange-500 to-orange-600 rounded-2xl blur-xl opacity-0 group-hover:opacity-50 transition-opacity duration-500"></div>
                          <div className="relative bg-white rounded-2xl p-4 sm:p-6 md:p-8 shadow-xl transform transition-all duration-500 hover:scale-105">
                            <img
                              src={partner.url}
                              alt={partner.name}
                              className="h-12 w-20 sm:h-16 sm:w-24 md:h-20 md:w-32 object-contain"
                            />
                          </div>
                        </div>
                      </div>
            ...
            <div className="flex justify-center items-center gap-2 mt-8">
              {Array.from({ length: Math.ceil(otherPartners.length / 5) }).map((_, index) => (
                <button
                  key={index}
                  onClick={() => setCurrentPartnerIndex(index * 5)}
                  className={`h-2 rounded-full transition-all duration-500 ${
                    Math.floor(currentPartnerIndex / 5) === index
                      ? 'bg-gradient-to-r from-orange-500 to-orange-600 w-12 shadow-lg shadow-orange-500/50'
